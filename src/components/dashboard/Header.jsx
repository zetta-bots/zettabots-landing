            ...
            <div style={{
              ...
              <div style={{ padding: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'linear-gradient(135deg, #7c3aed, #4f46e5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', fontWeight: '800', color: '#fff' }}>
                    {session?.name?.[0]?.toUpperCase() || session?.instanceName?.[0]?.toUpperCase() || 'U'}
                  </div>
                  <div>
                    <h4 style={{ margin: 0, fontSize: '0.95rem', fontWeight: '800', color: '#fff' }}>{session?.name || session?.instanceName || 'Usuário'}</h4>
                    <p style={{ margin: 0, fontSize: '0.75rem', color: '#64748b', fontWeight: '600' }}>{session?.email || 'zetta@bots.ia'}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Logout */}
              <div style={{ padding: '0.75rem' }}>
                <button style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '1rem 1.25rem', borderRadius: '16px', background: 'transparent', color: '#f87171', border: 'none', cursor: 'pointer', transition: 'all 0.2s', fontSize: '0.9rem', fontWeight: '600' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(239, 68, 68, 0.1)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'} onClick={() => { setShowUserMenu(false); window.location.href = '/'; }}>
                  <span style={{ fontSize: '1rem' }}>🚪</span>
                  Sair da Conta
                </button>
              </div>
