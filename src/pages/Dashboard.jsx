...
export default function Dashboard() {
  ...
  // Prompt Padrão da Sarah (O Cérebro)
...
5. Baseie suas respostas nos links de conhecimento fornecidos.`;
  ...
  useEffect(() => {
    ...
    // Recupera o prompt do cache ou usa o padrão
  ...
  const fetchQrCode = async (instanceName) => {
    ...
    try {
      ...
      if (isConnected) {
        ...
        // Busca estatísticas reais da Evolution
        ...
        if (current) {
          ...
          // Fallback para simulação visual se a API não retornar números reais
          setStats({ contacts: 124, chats: 42, messages: 856 })
        }
      } else {
  ...
  const fetchChatMessages = async (remoteJid) => {
    try {
      ...
      setChatMessages(rawMsgs.reverse().map(m => {
        const text = m.message?.conversation || m.message?.extendedTextMessage?.text || m.content || 'Mensagem de mídia';
        return {
          text: text,
      ...
      <main className="dashboard-main">
            ...
            <div className="glass-card">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginTop: '1rem' }}>
                <div className="onboarding-step-mini" style={{ borderLeft: qrStatus === 'CONNECTED' ? '4px solid #10b981' : '4px solid #7c3aed' }}>
                  <strong>WhatsApp</strong><p>{qrStatus === 'CONNECTED' ? '✅ Conectado' : '❌ Desconectado'}</p>
                </div>
                <div className="onboarding-step-mini" style={{ borderLeft: prompt.length > 100 ? '4px solid #10b981' : '4px solid #7c3aed' }}>
                  <strong>Cérebro IA</strong><p>{prompt.length > 100 ? '✅ Treinado' : '❌ Configurar'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card">
            ...
            <div className="table-container" style={{ marginTop: '1.5rem' }}>
              <table className="leads-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {leads.map((lead, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}><td style={{ padding: '1rem' }}>{lead.name}</td><td style={{ padding: '1rem' }}>{lead.phone}</td><td style={{ padding: '1rem' }}><span style={{ color: '#10b981', background: 'rgba(16,185,129,0.1)', padding: '4px 8px', borderRadius: '6px', fontSize: '0.8rem' }}>{lead.status}</span></td></tr>
                  ))}
                  {leads.length === 0 && <tr><td colSpan="3" style={{ padding: '2rem', textAlign: 'center', color: '#a1a1aa' }}>Buscando leads na Evolution API...</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {activeTab === 'bot' && (
          <div className="glass-card">
            ...
            <textarea className="prompt-editor" rows="18" value={prompt} onChange={(e) => setPrompt(e.target.value)} style={{ width: '100%', marginTop: '1rem', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', padding: '1.5rem', borderRadius: '12px', lineHeight: '1.6' }} />
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}><button onClick={handleSavePrompt} disabled={saving} style={{ padding: '1rem 2rem', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>{saving ? 'Salvando...' : 'Salvar e Treinar IA'}</button></div>
          </div>
        )}
        ...
        {activeTab === 'integracoes' && (
          <div className="glass-card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginTop: '1.5rem' }}>
              <div className="integration-card" style={{ padding: '1.5rem', background: 'rgba(255,255,255,0.03)', borderRadius: '20px', border: '1px solid rgba(124, 58, 237, 0.3)' }}>
                <h4>📧 Notificações por E-mail</h4>
                <p style={{ fontSize: '0.8rem', color: '#a1a1aa', margin: '0.5rem 0 1.5rem' }}>Receba um e-mail toda vez que a Sarah capturar um novo lead.</p>
                ...
                <button style={{ width: '100%', padding: '0.8rem', background: '#7c3aed', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '600' }} onClick={() => alert('Configurado! Você receberá alertas em ' + session.email)}>Ativar Alertas</button>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'conexao' && (
          <div className="glass-card" style={{ textAlign: 'center' }}>
            ...
            <div style={{ background: 'white', padding: '2rem', borderRadius: '20px', display: 'inline-block', marginTop: '2rem' }}>
              {qrStatus === 'CONNECTED' ? <div style={{ color: '#10b981', width: '250px' }}>✅ Conectado</div> : (qrCode ? <img src={qrCode} alt="QR" style={{ width: '250px' }} /> : <div className="spinner"></div>)}
            </div>
          </div>
        )}
      </main>
